 ...
 */
export const KNOWN_SHIP_TYPES = new Set(shipData.shipTypes.map((shipType) => shipType.id));
export const SAVE_VERSION = 1;
export const CURRENT_MAP_GRID = { width: 300, height: 180 };
...
export function validateSaveData(data, knownShipTypes = KNOWN_SHIP_TYPES) {
    const errors = [];
    const warnings = [];
    ...
    if (save.saveVersion !== undefined &&
        ...
        errors.push(error('save_version_invalid', 'saveVersion', `saveVersion must be an integer between 0 and ${SAVE_VERSION}`));
    }
            ...
            if (!faction.currencies ||
                ...
                for (const [key, value] of Object.entries(faction.currencies)) {
                    if (typeof value !== 'number' || !Number.isFinite(value)) {
                        errors.push(error('currency_nan', `${path}.currencies.${key}`, `currency value must be a finite number`));
                    }
                }
            }
            if (faction.researchedTechnologies !== undefined) {
                if (!Array.isArray(faction.researchedTechnologies)) {
                    errors.push(error('research_not_array', `${path}.researchedTechnologies`, 'researchedTechnologies must be an array'));
                }
                else if (faction.researchedTechnologies.some((t) => typeof t !== 'string')) {
                    errors.push(error('research_invalid_entry', `${path}.researchedTechnologies`, 'researchedTechnology entries must be strings'));
                }
            }
        }
        if (!playerFound) {
                ...
                if (!Array.isArray(system.planetsTiles)) {
                    ...
                    for (const [j, planet] of system.planetsTiles.entries()) {
                        ...
                        if (planet.satisfaction !== undefined &&
                            ...
                            errors.push(error('satisfaction_invalid', `${planetPath}.satisfaction`, 'satisfaction must be a finite number in [0, 100]'));
                        }
                        if (planet.shieldPoolCurrent !== undefined &&
                            ...
                            errors.push(error('shield_current_invalid', `${planetPath}.shieldPoolCurrent`, 'shieldPoolCurrent must be a non-negative finite number'));
                        }
