...
import { PLANET_TYPE_HABITABILITY, } from '../components/star-map/star-map.models';
const STOCK_RESOURCES = ['credits', 'rawmaterials', 'research'];
let EconomyService = class EconomyService {
    shipService;
    buildingStats = new Map();
    buildingStatsByName = new Map();
    ...
    POPULATION_GROWTH_BASE_RATE = 0.005;
    ...
    calculatePlanetEconomy(planet) {
        ...
        const workforceEfficiency = required > 0 ? Math.min(1, Math.max(0, available / required)) : 1;
        ...
        for (const building of planet.buildings ?? []) {
            ...
            for (const [resource, amount] of Object.entries(stats.production ?? {})) {
                production[resource] =
                    (production[resource] ?? 0) + amount * workforceEfficiency;
            }
     ...
     */
    calculateEconomy(factionId, factions, starSystems, fleets) {
        ...
        const planets = [];
        const fleetExpenses = [];
        const factionProduction = {};
        const factionConsumption = {};
        const factionNet = {};
        let totalExpense = 0;
     ...
     */
    applyEconomyDelta(factionId, factions, starSystems, fleets, deltaTime) {
    ...
    findPlanetByName(planetName, starSystems) {
     ...
     */
    getPlanetEconomyBreakdown(planet) {
        const economy = this.calculatePlanetEconomy(planet);
        const pop = planet.population || 0;
        const planetExpense = this.calculatePlanetMaintenance(planet);
        const buildings = this.getPlanetBuildingExpenses(planet);
    ...
    calculatePlanetMaintenance(planet) {
        let expense = 0;
        for (const building of planet.buildings ?? []) {
            const stats = this.buildingStatsByName.get(building.name);
    ...
    getPlanetBuildingExpenses(planet) {
        const buildingCounts = new Map();
        const buildingCosts = new Map();
        for (const building of planet.buildings ?? []) {
            const stats = this.buildingStatsByName.get(building.name);
            const cost = stats?.maintenanceCost ?? 0;
            if (cost > 0) {
    ...
    getOwnedPlanets(factionId, starSystems) {
        const planets = [];
        for (const system of starSystems) {
            for (const planet of system.planetsTiles ?? []) {
