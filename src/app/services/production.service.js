...
let ProductionService = class ProductionService {
    static { ProductionService_1 = this; }
    shipService;
    shipStockService;
    static STALLED_ORDER_TIMEOUT = 30;
    static DEFAULT_FACTORY_POWER = 0.5;
    static DEFAULT_FACTORY_SLOTS = 1;
    orderIdCounter = 1;
    tickCounter = 0;
    constructor(shipService, shipStockService) {
     ...
     */
    queueOrder(data, factionId, planetId, shipTypeId, quantity, starSystems, factions) {
        if (quantity <= 0) {
     ...
     */
    cancelOrder(data, factionId, planetId, orderId, factions) {
    ...
    getQueue(data, factionId, planetId) {
        if (!data.production) {
            data.production = [];
        }
    ...
    getPlanetCapacity(planet, kind = 'spaceship_factory') {
        return planet.buildings
            .filter((b) => b.name === 'Spaceship Factory' && kind === 'spaceship_factory')
            .reduce((sum, b) => sum + ProductionService_1.DEFAULT_FACTORY_SLOTS, 0);
    }
    getPlanetPower(planet, kind = 'spaceship_factory') {
        return planet.buildings
            .filter((b) => b.name === 'Spaceship Factory' && kind === 'spaceship_factory')
            .reduce((sum) => sum + ProductionService_1.DEFAULT_FACTORY_POWER, 0);
    }
    /*
     ...
     */
    tick(deltaTime, data, starSystems, fleets, factions) {
        if (deltaTime <= 0) {
        ...
        for (const factionProd of data.production) {
            for (const planetIdStr of Object.keys(factionProd.ordersByPlanet)) {
                ...
                if (power <= 0) {
                    ...
                    if (age > ProductionService_1.STALLED_ORDER_TIMEOUT) {
                        queue.shift();
                        const refund = this.refundOrder(head, factions, factionProd.factionId);
                        result.refundedOrders.push({ factionId: factionProd.factionId, planetId, orderId: head.id, refunded: refund });
                        result.stateChanged = true;
                ...
                if (head.progress >= 1) {
                    ...
                    for (let i = 0; i < head.quantity; i++) {
                        entries.push({
                            id: this.shipStockService.nextShipId(fleets, data),
                            type: shipType.id,
                            name: shipType.name,
                    ...
                    this.shipStockService.addToStock(data, factionProd.factionId, entries);
    ...
    refundOrder(order, factions, factionId) {
        const faction = factions.find((f) => f.id === factionId);
        if (!faction) {
    ...
    findPlanet(starSystems, planetId) {
