 ...
 */
let ShipStockService = class ShipStockService {
    static { ShipStockService_1 = this; }
    static DEFAULT_SHIP_NAME = 'Ship';
    /*
    ...
    getCount(data, factionId, typeId) {
     ...
     */
    addToStock(data, factionId, entries) {
        if (entries.length === 0) {
            return;
        }
     ...
     * is used.
     */
    removeFromStock(data, factionId, typeId, count) {
        if (count <= 0) {
            return [];
        }
        const stock = this.getStock(data, factionId);
     ...
     * just drops them.
     */
    onFactionRemoved(data, factionId) {
        if (!data.shipStock) {
            return [];
        }
    ...
    static defaultNameFor(_typeName) {
        return ShipStockService_1.DEFAULT_SHIP_NAME;
    }
    /*
     ...
     */
    systemGalaxyCell(system) {
        return { x: system.x, y: system.y };
    }
};
ShipStockService = ShipStockService_1 = __decorate([
    Injectable({ providedIn: 'root' })
], ShipStockService);
export { ShipStockService };
