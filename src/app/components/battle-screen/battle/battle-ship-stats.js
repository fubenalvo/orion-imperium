...
function fallbackTier(cost) {
    return Math.min(5, Math.max(1, Math.ceil(cost / 150)));
}
/* Virtual defense building attack ranges (planet-data.json defense block). */
const VIRTUAL_RANGE_LOOKUP = Object.fromEntries(planetData.buildings
    .filter((b) => b.range != null)
    .map((b) => [b.id, b.range]));
export function getBattleShipStats(typeId, shipService, planetBattleService) {
