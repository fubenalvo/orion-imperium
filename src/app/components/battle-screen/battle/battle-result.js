 ...
 */
export function buildBattleOutcome(state) {
    const attacker = fleetOutcome(state.attackerFleetId, state.attackerFactionId, 'attacker', state.attackerShips);
    const defender = fleetOutcome(state.defenderFleetId, state.defenderFactionId, 'defender', state.defenderShips);
    ...
    const winnerSide = state.winner ?? 'attacker';
...
function fleetOutcome(fleetId, factionId, side, roster) {
    const ships = roster.map((s) => ({
        shipId: s.shipId,
        typeId: s.typeId,
        name: s.name,
