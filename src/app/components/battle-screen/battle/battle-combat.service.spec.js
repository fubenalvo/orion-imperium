...
describe('BattleCombatService', () => {
    ...
    const fleetShip = (id, type) => ({ id, name: `S${id}`, type });
    ...
    it('applies max(1, totalAttack - frontDefense) to the front shield', async () => {
        const state = setup(Array.from({ length: 5 }, (_, i) => fleetShip(i, 'fighter')), [fleetShip(100, 'frigate')]);
    ...
    it('spills overkill across a defender stack until a ship dies', async () => {
        const state = setup(Array.from({ length: 5 }, (_, i) => fleetShip(i, 'fighter')), [fleetShip(100, 'scout'), fleetShip(101, 'scout')]);
        const atk = mergeSameTypeStacks(state, 'attacker');
        ...
        const wounded = def.ships.filter((s) => s.alive && (s.hp < s.maxHp || (s.shield ?? 0) < (s.maxShield ?? 0)));
    ...
    it('destroys a stack when all its ships die and triggers victory', async () => {
        const state = setup(Array.from({ length: 5 }, (_, i) => fleetShip(i, 'fighter')), [fleetShip(100, 'scout')]);
        const atk = mergeSameTypeStacks(state, 'attacker');
        const def = getStacks(state, 'defender')[0];
        placeAdjacent(atk, def);
        const p = combat.attackStack(state, atk.stackId, def.stackId);
    ...
    it('rejects attacking a same-side stack', async () => {
        const state = setup([fleetShip(0, 'fighter'), fleetShip(1, 'scout')], [fleetShip(100, 'frigate')]);
    ...
    it('depletes the shield and applies overflow to hull HP', async () => {
        const state = setup([fleetShip(0, 'cruiser'), fleetShip(1, 'cruiser'), fleetShip(2, 'cruiser')], [fleetShip(100, 'scout')]);
        const atk = mergeSameTypeStacks(state, 'attacker');
        const def = getStacks(state, 'defender')[0];
        placeAdjacent(atk, def);
    ...
    it('applies shield to each ship in order across a multi-ship stack', async () => {
        const state = setup([fleetShip(0, 'cruiser'), fleetShip(1, 'cruiser'), fleetShip(2, 'cruiser')], [fleetShip(100, 'scout'), fleetShip(101, 'scout')]);
        const atk = mergeSameTypeStacks(state, 'attacker');
        const def = mergeSameTypeStacks(state, 'defender');
        placeAdjacent(atk, def);
        def.ships[0].shield = 20;
    ...
    it('restores shieldRegen to a damaged friendly stack in range', () => {
        const state = setup([fleetShip(0, 'carrier'), fleetShip(1, 'fighter')], [fleetShip(100, 'frigate')]);
     ...
     */
    const planetSetup = (defender, shieldPool, shieldPoolRegen) => {
        const state = createBattleState({
            ...
            type: 'planet',
            planetId: 5,
        }, shipService, planetBattleService);
        anim.reset();
        return state;
    };
    ...
    it('does not protect mobile garrison stacks with the shared planetary shield', async () => {
        ...
        expect(garrison.ships[0].hp + (garrison.ships[0].shield ?? 0)).toBeLessThan(garrison.ships[0].maxHp + (garrison.ships[0].maxShield ?? 0));
    });
