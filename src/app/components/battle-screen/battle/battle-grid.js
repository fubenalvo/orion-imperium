...
export function weaponMultiplier(attackerType, targetWeakness) {
    const row = WEAPON_EFFECTIVENESS[attackerType];
    if (!row) {
        return 1.0;
    }
    return row[targetWeakness] ?? 1.0;
}
/*
 ...
 */
export function applyShieldRegen(current, max, regen) {
    if (regen <= 0 || max <= 0) {
        return current;
    }
...
export function computeTargetScore(attacker, target, distance) {
...
export function isOccupied(state, col, row, excludeStackId) {
    return state.stacks.some((s) => !s.destroyed && s.stackId !== excludeStackId && occupiesCell(s, col, row));
}
export function occupiesCell(stack, col, row) {
    if (stack.row !== row) {
        return false;
    }
...
export function getOccupiedCells(stack) {
    const cells = [];
    const direction = stack.side === 'attacker' ? 1 : -1;
    for (let i = 0; i < stack.size; i++) {
        cells.push({
...
export function getStackAt(state, col, row) {
    return (state.stacks.find((s) => !s.destroyed && occupiesCell(s, col, row)) ?? null);
}
export function getAliveStackCount(state, side) {
...
export function stackCenterVw(stack) {
...
export function vwToStackCell(stack, x, y) {
 ...
 */
export function regenerateAllShields(state) {
 ...
 */
export function updateStackPositions(state, deltaTime) {
    for (const stack of state.stacks) {
        if (stack.destroyed || stack.targetX == null || stack.targetY == null)
            continue;
 ...
 */
export function getReachableCells(state, stack) {
    if (stack.destroyed || stack.immobile) {
        return [];
    }
    const origin = { col: stack.col, row: stack.row };
    const cells = [];
...
function occupiedCols(stack, anchorCol) {
    const cols = [];
    const direction = stack.side === 'attacker' ? 1 : -1;
    for (let i = 0; i < stack.size; i++) {
...
export function isPathClear(state, path, stack) {
...
export function getAttackTargetIds(state, stack) {
    const origin = { col: stack.col, row: stack.row };
    return state.stacks
 ...
 */
export function computeCarrierBoostTargets(state, carrier) {
    if (carrier.typeId !== 'carrier' || carrier.destroyed) {
        return [];
    }
    const origin = { col: carrier.col, row: carrier.row };
    return state.stacks
 ...
 */
export function getMoveToAttackCells(state, stack, targetStack) {
 ...
 */
export function findBestMoveToAttackCell(state, stack, targetStack) {
 ...
 */
export function getMoveToAttackTargetIds(state, stack) {
    if (stack.destroyed || stack.immobile) {
        return [];
    }
    const origin = { col: stack.col, row: stack.row };
