 ...
 */
let BattleAiService = class BattleAiService {
    combat;
    movement;
    anim;
    constructor(combat, movement, anim) {
        this.combat = combat;
    ...
    getAiStacks(state) {
        return state.stacks.filter((s) => !s.destroyed && !isSidePlayerControlled(state, s.side));
    }
    bestTarget(state, stack) {
        const origin = { col: stack.col, row: stack.row };
        const candidates = state.stacks.filter((s) => !s.destroyed && s.side !== stack.side && isInRange(origin, s, stack.attackRange));
    ...
    async moveTowardNearestEnemy(state, stack) {
        const origin = { col: stack.col, row: stack.row };
        ...
        const moveAttackTarget = state.stacks
            ...
            .sort((a, b) => cellDistance(origin, a) - cellDistance(origin, b) ||
            a.stackId.localeCompare(b.stackId))[0];
        if (moveAttackTarget) {
        ...
        const enemy = state.stacks
            .filter((s) => !s.destroyed && s.side !== stack.side)
            .sort((a, b) => cellDistance(origin, a) - cellDistance(origin, b) || a.stackId.localeCompare(b.stackId))[0];
        if (!enemy) {
            return false;
        }
...
BattleAiService = __decorate([
    Injectable({ providedIn: 'root' })
], BattleAiService);
export { BattleAiService };
