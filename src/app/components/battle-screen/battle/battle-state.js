import { MAX_STACK_SIZE, } from './battle.types';
import { ATTACKER_DEPLOY_COLS, DEFENDER_DEPLOY_COLS, BATTLE_GRID_COLUMNS } from './battle.types';
import { getBattleShipStats } from './battle-ship-stats';
import { isInBounds, stackCenterVw } from './battle-grid';
/*
 * =========================================================
...
export function createBattleState(battle, shipService, planetBattleService) {
    ...
    const shieldPoolMax = battle.type === 'planet' ? (defenderFleet.shieldPoolMax ?? defenderFleet.shieldPool ?? 0) : 0;
    return {
        round: 1,
        ...
        defenderShieldPool: battle.type === 'planet' && shieldPoolMax > 0
...
function sideFleet(battle, fleetId) {
    return battle.fleet1.id === fleetId ? battle.fleet1 : battle.fleet2;
}
function toBattleShip(ship, shipService, planetBattleService) {
    ...
    if (!recognized || cameInDestroyed) {
        ...
        const persisted = typeof ship.currentHp === 'number' && Number.isFinite(ship.currentHp)
            ? ship.currentHp
            : stats.maxHp;
...
function buildStacks(side, roster, shipService, planetBattleService) {
...
function deployStacks(stacks, columns) {
 ...
 */
function deployDefenderStacks(stacks) {
    ...
    const garrisonStartIndex = Math.min(lastDefenseColumnIndex + 1, DEFENDER_DEPLOY_COLS.length);
...
function canPlaceDefenderStack(stack, col, row, occupied) {
...
function placeDefenderStack(stack, col, row, occupied) {
    stack.col = col;
    stack.row = row;
    for (let offset = 0; offset < stack.size; offset++) {
...
function placeDefenderStackAnywhere(stack, occupied) {
...
function initializeStackPositions(stacks) {
...
export function getStacks(state, side) {
    return state.stacks
 ...
 */
export function isSidePlayerControlled(state, side) {
