...
import { checkVictory, computeCarrierBoostTargets, isInRange, stackCenterVw, weaponMultiplier, applyShieldRegen, } from './battle-grid';
/*
 * =========================================================
 ...
 */
let BattleCombatService = class BattleCombatService {
    anim;
    constructor(anim) {
        this.anim = anim;
    }
    async attackStack(state, attackerStackId, targetStackId) {
        ...
        await this.anim.run(async () => {
            ...
            if (front) {
                ...
                const multiplier = weaponMultiplier(attackerFront?.attackType ?? 'kinetic', front.weakness ?? 'energy');
     ...
     */
    carrierShieldBoost(state, carrierStackId) {
...
BattleCombatService = __decorate([
    Injectable({ providedIn: 'root' })
], BattleCombatService);
export { BattleCombatService };
