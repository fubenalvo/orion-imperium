...
let EnemyAiService = class EnemyAiService {
    shipService;
    ...
    tick(gameDeltaTime, fleets, factions) {
        if (gameDeltaTime <= 0) {
            return false;
        }
        ...
        for (const fleet of fleets) {
            ...
            let targetedPlayerFleet;
            const targetedPlayerFleetId = this.currentTargets.get(fleet.id);
            if (targetedPlayerFleetId !== undefined) {
                targetedPlayerFleet = fleets.find((candidate) => candidate.id === targetedPlayerFleetId);
            }
            else if (fleet.targetX !== null && fleet.targetY !== null) {
                targetedPlayerFleet = playerFleets.find((candidate) => candidate.x === fleet.targetX && candidate.y === fleet.targetY);
