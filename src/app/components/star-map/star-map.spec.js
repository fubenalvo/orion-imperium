...
describe('StarMap', () => {
    ...
    describe('EnemyActionService integration', () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(economyService, 'applyEconomyDelta').mockImplementation(() => ({}));
            vi.spyOn(economyService, 'calculateEconomy').mockImplementation(() => ({}));
            // Suppress verbose AI console output and prevent Angular CD.
            logSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
            vi.spyOn(component['ngZone'], 'run').mockImplementation((fn) => fn());
        ...
        it('should execute produce_colonizer through the full AI pipeline', () => {
            ...
            component['gameLoopCallback'](2);
            expect(queueOrderSpy).toHaveBeenCalledWith({ production: component.production }, 'enemy1', 1, 'colonizer', 1, component.starSystems, component.factions);
        ...
        it('should execute assemble_fleet through the full AI pipeline', () => {
            ...
            expect(raider.ships.some((s) => s.type === 'colonizer' && s.id === 5001)).toBe(true);
            const enemyStock = component.shipStock.find((s) => s.factionId === 'enemy1');
            expect(enemyStock?.ships.filter((s) => s.type === 'colonizer') ?? []).toHaveLength(0);
            component['gameLoopCallback'](2);
            expect(component.fleets.find((f) => f.id === 3).ships.filter((s) => s.type === 'colonizer')).toHaveLength(1);
        });
    });
