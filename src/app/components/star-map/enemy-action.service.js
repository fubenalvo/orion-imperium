 ...
 */
let EnemyActionService = class EnemyActionService {
    shipService;
    researchService;
    shipStockService;
    productionService;
    ...
    constructor(shipService, researchService, shipStockService, productionService, spaceportService, fleetAssemblyService, movementService) {
    ...
    tick(gameDeltaTime, currentGoal, capability, factionId, fleets, factions, starSystems, shipStock, production) {
        if (gameDeltaTime <= 0) {
            return false;
        }
    ...
    evaluateAction(goal, capability, factionId, fleets, factions, starSystems, shipStock, production) {
        /*
    ...
    evaluatePreparationAction(goal, capability, factionId, fleets, factions, starSystems, shipStock, production) {
        switch (goal.type) {
            case 'colonize':
    ...
    evaluateColonizePrepare(goal, capability, factionId, fleets, factions, starSystems, shipStock, production) {
    ...
    evaluateColonizeExecute(goal, factionId, fleets, starSystems, shipStock) {
    ...
    evaluateAttackPrepare(goal, capability, factionId, fleets, factions) {
        const reqMap = new Map(capability.requirements.map((r) => [r.type, r]));
    ...
    evaluateAttackExecute(goal, factionId, fleets) {
    ...
    evaluateDefendPrepare(goal, capability, factionId, fleets, factions, starSystems, shipStock) {
        const reqMap = new Map(capability.requirements.map((r) => [r.type, r]));
        const targetValid = reqMap.get('target_valid');
    ...
    evaluateDefendExecute(goal, factionId, fleets, starSystems) {
        const system = starSystems.find((s) => s.id === goal.targetSystemId);
        if (!system) {
    ...
    findFleetWithColonizer(factionId, fleets) {
        const candidates = fleets.filter((f) => f.factionId === factionId && !f.destroyed && f.ships.some((s) => s.type === 'colonizer' && !s.destroyed));
    ...
    findBestEnemyFleet(factionId, fleets) {
        const candidates = fleets.filter((f) => f.factionId === factionId && !f.destroyed && f.ships.some((ship) => !ship.destroyed));
        if (candidates.length === 0) {
            return undefined;
        }
        candidates.sort((a, b) => {
    ...
    canProduceColonizer(factionId, factions, starSystems) {
        ...
        const enemySystems = starSystems.filter((s) => s.planetsTiles.some((p) => p.factionId === factionId));
    ...
    selectCheapestCombatShip(factionId, factions) {
        const faction = factions.find((f) => f.id === factionId);
        if (!faction) {
            return undefined;
        }
     ...
     */
    selectStockCombatShip(factionId, shipStock) {
    ...
    canAffordReinforcement(factionId, factions) {
        const faction = factions.find((f) => f.id === factionId);
        if (!faction) {
            return false;
        }
    ...
    selectWeakestFleet(factionId, fleets) {
        const factionFleets = fleets.filter((fleet) => fleet.factionId === factionId && !fleet.destroyed && fleet.ships.length > 0);
        if (factionFleets.length === 0) {
            return undefined;
        }
        factionFleets.sort((a, b) => this.shipService.calculateFleetStrength(a.ships) - this.shipService.calculateFleetStrength(b.ships));
     ...
     */
    selectReinforcementPlanet(factionId, starSystems) {
    ...
    createAction(type, factionId, goal, targetId, targetSystemId, targetPlanetId, reason, shipTypeId) {
        return {
            type,
            factionId,
            goalType: goal.type,
    ...
    createNoneAction(factionId, goal, reason) {
        return {
            type: 'none',
            factionId,
...
EnemyActionService = __decorate([
    Injectable({ providedIn: 'root' })
], EnemyActionService);
export { EnemyActionService };
