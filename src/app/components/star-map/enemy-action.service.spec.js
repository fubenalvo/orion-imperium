...
describe('EnemyActionService', () => {
    ...
    let shipService;
    let researchService;
    let shipStockService;
    let productionService;
    let spaceportService;
    ...
    beforeEach(() => {
        ...
        spaceportService = {
            ...
            isSpaceportPlanet: vi.fn((planet) => (planet?.buildings ?? []).some((b) => b.name === 'Spaceport')),
            listSpaceports: vi.fn((factionId, starSystems) => {
                const out = [];
                for (const system of starSystems ?? []) {
                    for (const planet of system.planetsTiles ?? []) {
                        if (planet.factionId === factionId && (planet.buildings ?? []).some((b) => b.name === 'Spaceport')) {
    ...
    const makeCapability = (overrides = {}) => ({
        canExecute: true,
        goalType: 'colonize',
        factionId: 'enemy1',
        requirements: [],
    ...
    const makeColonizeGoal = (overrides = {}) => ({
        type: 'colonize',
        targetPlanetId: 1,
        targetSystemId: 'sys1',
        ...overrides,
    });
    const makeAttackGoal = (overrides = {}) => ({
        type: 'attack',
        targetFleetId: 1,
        ...overrides,
    });
    const makeDefendGoal = (overrides = {}) => ({
        type: 'defend',
        targetPlanetId: 1,
        targetSystemId: 'sys1',
        ...overrides,
    });
    const makeDevelopGoal = () => ({ type: 'develop' });
    describe('colonize goal', () => {
        it('should produce PRODUCE_COLONIZER when colonizer is missing but production is possible', () => {
            const factionsWithCredits = baseFactions.map((f) => f.id === 'enemy1' ? { ...f, currencies: { credits: 200 }, researchedTechnologies: ['basic_engineering'] } : f);
            const systems = [
                createSystem({
                    id: 'sys1',
                    x: 10,
        ...
        it('should produce ASSEMBLE_FLEET when colonizer is in stock but no fleet has it', () => {
            const factionsWithCredits = baseFactions.map((f) => f.id === 'enemy1' ? { ...f, researchedTechnologies: ['basic_engineering'] } : f);
            ...
            shipStockService.getCount.mockReturnValue(1);
        ...
        it('should produce MOVE_TO_TARGET when fleet with colonizer is away from target', () => {
            ...
            service = new EnemyActionService(shipService, researchService, shipStockService, productionService, spaceportService, fleetAssemblyService, movementMock);
            const fleets = [
                createFleet({
                    factionId: 'enemy1',
                    name: 'RAIDER',
    ...
    describe('attack goal', () => {
        ...
        it('should produce PRODUCE_COMBAT_SHIP when unable to engage but combat ship production is possible', () => {
            const factionsWithCredits = baseFactions.map((f) => f.id === 'enemy1' ? { ...f, currencies: { credits: 500 }, researchedTechnologies: ['basic_engineering'] } : f);
            const systems = [
                createSystem({
                    id: 'sys1',
                    x: 10,
