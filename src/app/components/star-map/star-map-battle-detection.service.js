 ...
 */
let StarMapBattleDetectionService = class StarMapBattleDetectionService {
    battleService;
     ...
     */
    checkForBattles(fleets, factions, calculateGridCell, isFleetInSystem, starSystems, enterBattleScreen, triggeredBattles) {
        ...
        for (let i = 0; i < activeFleets.length; i++) {
            for (let j = i + 1; j < activeFleets.length; j++) {
                ...
                const attackerId = fleet1.targetX !== null && fleet1.targetY !== null ? fleet1.id : fleet2.id;
...
StarMapBattleDetectionService = __decorate([
    Injectable({ providedIn: 'root' })
], StarMapBattleDetectionService);
export { StarMapBattleDetectionService };
